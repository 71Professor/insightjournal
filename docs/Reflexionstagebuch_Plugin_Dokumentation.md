...
## **1 Moodle Activity Module · Version 0.7.1-beta · Juli 2026**
...
- **Behat-Testabdeckung ist begrenzt:** Fünfzehn Szenarien decken ein normales Formular-Absenden ganz ohne JavaScript, den Speicher-/Neuladen-Ablauf, das Bearbeiten einer gespeicherten Antwort, Autosave, die Mindestzeichenzahl-Abschlussregel, den Erfolgsstatus beim Speichern, das private Markieren eines einzelnen Eintrags durch die/den Lernende/n, den Atto-Editor, unterschiedliche Sichtbarkeits-Entscheidungen über mehrere Aktivitäten hinweg, einen abgelehnten Speicherkonflikt mit anschließender Sperre bis zum Neuladen, die Pagination beider Berichte sowie die Gruppentrennung (Separate Groups) auf allen drei Berichts-/Zusammenfassungsseiten ab. Umfassendere Abdeckung (CSV-Export) ist noch nicht automatisiert.
- **Die Gruppentrennung ist auf der Zusammenfassungsseite gröber als im Aktivitätsbericht.** `report.php` schränkt pro Aktivität ein – da immer nur eine Aktivität betroffen ist, ist das immer korrekt granular. `summary.php`s Prüfung beim Ansehen einer/eines anderen Lernenden schränkt hingegen pro Kurs ein: Wenn irgendeine für die/den Betrachter/in sichtbare Aktivität Gruppentrennung nutzt, wird der Zugriff auf jede Zusammenfassung gesperrt, nicht nur auf Einträge dieser einen Aktivität – analog zur bereits bestehenden, bewusst groben Einschränkung des Kursgesamtberichts. In einem Kurs mit gemischten Gruppenmodi über mehrere Insight-Journal-Aktivitäten hinweg kann `report.php` daher einen Teilnehmenden-Link anzeigen, den `summary.php` anschließend ablehnt. Fail-closed (es werden nie Daten offengelegt); gefunden beim abschließenden Review von R2-05 (2026-07-28). Geplant: `summary.php`s Prüfung auf die konkrete Aktivität eingrenzen, von der aus zugegriffen wird.
...
*Erstellt: Juli 2026 · Plugin: mod_insightjournal v0.7.1-beta*
