...
### R4-03 · Gruppenautorisierung speicherbegrenzt machen  `[FR]` — ✅ Erledigt (2026-08-05, `main`, noch nicht gepusht)
...
**Verifiziert:** PHPUnit 217/217 grün (keine „undefined function"-Fehler nach dem Entfernen der beiden Alt-Funktionen), phpcs sauber (`moodle`+`moodle-extra`, `--warning-severity=1`), PHPStan 0 Fehler, Behat grün (`@mod_insightjournal`). Vorher/Nachher-Benchmark (identischer Seed: der Viewer gehört zu genau einer Gruppe, deren eigene Mitgliederzahl auf 50/500/5000 wächst, plus einer kleinen Fremdgruppe) zeigt: die Queryzahl blieb in beiden Fällen bei 9 – der Gewinn liegt **nicht** in weniger Round-Trips, sondern darin, dass die in PHP materialisierte Ergebnismenge jetzt durch Seiten-/Chunkgröße (20 Zeilen bzw. ≤500 CSV-Zeilen) begrenzt ist statt 1:1 mit der Gruppengröße zu wachsen (vorher 51→501→5001 Einträge in der Lookup-Map). Die Laufzeit bestätigt das direkt: vorher wuchs sie von 0,0052 s (50 Mitglieder) auf 0,0434 s (5000 Mitglieder, ca. Faktor 8); nachher blieb sie bei allen drei Größen konstant bei ca. 0,006 s. Entspricht exakt dem Abnahmekriterium „Peak Memory proportional zum 500er-Chunk statt zur Kursgröße".
