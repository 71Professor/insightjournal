...
| **R4-03** | **WIEDER OFFEN** | Gruppen-IDs, EXISTS und Chunk-Maps begrenzen Speicher korrekt. Das direkte groups\_members-SQL hat aber Moodles Membership-Visibility nicht übernommen — neuer P0 R5-01. |
...
**Negativ.** CR-07 ist die Quelle des aktuellen CI-Fehlers. Die lokale Aussage „volle Behat-Suite grün“ in Fix.md gilt nicht für die GitHub-Matrix mit Moodles SCSS-Deprecation-Prüfung.

# **4  P1 — Stable-Gate und Release-Wahrheit**

| ID | Prio | Thema | Konkrete Änderung | Abnahme |
| ----- | ----- | :---- | :---- | :---- |
| **R5-03** | **P1** | **Untracked Template-Test bewusst auflösen** | Empfohlen: tests/report\_template\_test.php löschen, weil report\_table\_test.php die Datenschutzzeilen bereits am realen Renderer prüft. Alternativ als report\_shell\_template\_test neu schreiben und ausschließlich Back-Link, CSV-Link, Suche, cmid und actionurl testen. Keine synthetischen rows/private/response-Daten an report.mustache geben. | git status ist sauber; kein Test erwartet Datenzeilen aus dem Shell-Template. Ein echter Geheimtext wird nur dort geprüft, wo report\_table oder coursereport\_provider ihn tatsächlich verarbeitet. |
| **R5-04** | **P1** | **Dokumentation und Release-Paket synchronisieren** | README: Baseline-Verweis entfernen, 24 Behat-Szenarien und CSV-Integration aktualisieren. CHANGELOG: Zwischenzustand zu entfernten current\_user\_groups()-Funktionen konsolidieren und Integration statt E2E sagen. Fix.md: „uncommitted“ korrigieren und nach /docs verschieben oder per export-ignore ausschließen. Stale Baseline-/Trigger-Kommentare in ci.yml/release.yml korrigieren. | git archive HEAD enthält keine internen Arbeitsnotizen. README, CHANGELOG, version.php und CI-Beschreibung nennen denselben Test-/Release-Stand; ein Archiv-Inventartest bestätigt die Paketgrenzen. |
...
| **6** | R5-05 Zeichensemantik entscheiden und Fixtures erweitern. | PHPUnit-Fixtures, Browser-Parität, Completion-/maxchars-Behat. | Keine ungewollte minchars-Umgehung; Vertrag dokumentiert. |
| **7** | R5-06/R5-08/R5-09: Teilnehmervertrag, CSV-Smoke, Provider-Härtung. | PHPUnit, optional Behat-Download, PHPStan. | Paging/CSV/Capabilities konsistent; klare öffentliche Verträge. |
