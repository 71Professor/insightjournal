...
>
> **R5-07, R5-08, R5-09, R5-11 seit diesem Update ebenfalls ✅ erledigt.** R5-10 (ESM) ebenfalls ✅ erledigt (2026-08-07, Details siehe Tabelle unten und CHANGELOG `[Unreleased]`). Details je Punkt in `Fix.md`. **Damit ist der komplette R5-Backlog aus diesem Re-Review abgearbeitet.**
>
> **Verifiziert (letzter Stand):** PHPUnit 242/242 (PHPUnit-Deprecations 5 → 1), phpcs sauber, beide PHPStan-Configs 0 Fehler (der Test-PHPStan-Schritt ist jetzt ein echtes Gate statt continue-on-error), Behat-Vollauf angestoßen.
...
| **R5-10** ✅ | **P3** | **Neues JavaScript schrittweise auf ESM umstellen** | autosave.js/summary.js auf import/export-Syntax migriert. Beide Module exportieren jetzt benannt (`init`, bei autosave.js zusätzlich `visibleCharCount`/`wordCount`) statt als ein Default-Objekt – `js_call_amd()` ruft `amd.{func}(...)` direkt am geladenen Modul auf, und der Behat-Helper liest `autosave.visibleCharCount`/`.wordCount` genauso direkt; ein reiner `export default {...}` hätte beides zur Laufzeit stillschweigend gebrochen (im Zuge dieser Umsetzung selbst gefunden und korrigiert). Der TinyAdapter nutzt jetzt dynamisches `import()` statt `require()`. | Grunt erzeugt weiterhin ein AMD-ladbares Build (`define("mod_insightjournal/...", [...], ...)`); Tiny/Atto/Plain-Textarea-Behat bleibt grün (24/24, 365/365 Steps); phpcs sauber. Die lokale `Squiz.Functions.MultiLineFunctionDeclaration`-Ausnahme bleibt entgegen der ursprünglichen Erwartung bestehen – sie greift bei jedem mehrzeiligen `function`-Ausdruck, nicht nur beim define()-Wrapper (empirisch auch für die alte AMD-Fassung bestätigt), also keine neue globale Ausnahme, aber auch keine entfernte lokale. |
...
| **5** ✅ | R5-07 Test-PHPStan auf echten Nullfehler-Pilot bringen. | PHPStan JSON/Standardausgabe, neuer gating Schritt. | Erledigt: 3 echte Findings behoben, Pilot auf `tests/generator/lib.php`+`tests/behat/behat_mod_insightjournal.php` verengt (0 Fehler), `continue-on-error` in CI entfernt. |
...
| **7** ✅ (R5-06 dokumentiert, R5-08 dokumentiert, R5-09 implementiert) | R5-06/R5-08/R5-09: Teilnehmervertrag, CSV-Smoke, Provider-Härtung. | PHPUnit, optional Behat-Download, PHPStan. | R5-06: per Rückfrage dokumentiert statt geändert. R5-08: „Integrationstest" statt „E2E", optionaler HTTP-Smoke bereits in R4-09 als nicht praktikabel befunden. R5-09: `coding_exception` für ungültige chunksize/diary-id, 3 neue Tests. |
| **8** ✅ | R5-10/R5-11 Wartung: ESM, Pins, PHPUnit-Deprecations. | Grunt, Code Checker, volle CI, Behat. | R5-11: 3 CI-Actions + moodle-plugin-ci gepinnt, PHPUnit-Deprecations 5→1 (die letzte bewusst dokumentiert, PHPUnit-9.6-Kompatibilität). R5-10: autosave.js/summary.js auf ESM migriert (2026-08-07), benannte statt Default-Exports (Laufzeit-Bug sonst), Grunt/phpcs/Behat (24/24) grün. |
